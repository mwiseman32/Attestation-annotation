...
static void check_pcrten()
{
	...
	for (i=0; i< MAX_DATA_HASH; i++)
		if (pcrten[i] != pcrten_target[i])
			return;
	printf(" PCR-10 calculated matches target! ");
}
...
static void display_tcg_base(struct tlv *tlv)
{
	if (tlv->t == IMA_TLV_SEQ)
		printf("SEQNUM %08d ", *(uint32_t *)(tlv->v));
	else if (tlv->t == IMA_TLV_PCR)
		printf("PCRNUM %02d ", *(uint8_t *)(tlv->v));
		...
		printf("tcg_base %02d %02d ",tlv->t, tlv->l);
		hexdump(tlv->v, tlv->l);
	}
}
...
static void display_ima_base(struct tlv *tlv)
{
	...
	if (tlv->t == IMA_TLV_CONTENT_PATH) {
		...
		printf("Unregognized type %d len %d ", tlv->t, tlv->l);
}
...
static void display_ima_content(struct tlv *tlv)
{
	...
	if (!memcmp(content_hash, content_digest, SHA1_HASH_LEN)) {
		printf("Digest Matches content. ");
	} else {
		printf("SHA1 of content: ");
...
int main(int argc, char *argv[])
{
	...
	if (argc == 3) {
		if (argv[1][1] == 'k') {
			...
			pk_initialized = 1;
		#ifdef DEBUG_MSG
			printf("I am in arg 3 if: \n");
       	        #endif	
		} else if (argv[1][1] == 'p')
			init_pcr(argv[2]);	
		#ifdef DEBUG_MSG
			printf("I am in arg 3 else: \n");
       	        #endif		
	}		
	if (argc == 5) {
		if (argv[3][1] == 'k') {
			...
			pk_initialized = 1;
		#ifdef DEBUG_MSG
			printf("I am in arg 5 if: \n");
       	        #endif	
		} else if (argv[3][1] == 'p')
			init_pcr(argv[4]);	
		#ifdef DEBUG_MSG
			printf("I am in arg 5 else: \n");
       	        #endif			
	}	
	if (!pk_initialized)
		init_pk_list("./ima_pub.pem");
		#ifdef DEBUG_MSG
			printf("initialized ima_pub.pem: \n");
       	        #endif	
	#ifdef DEBUG_MSG
	...
	while (read_tcg_tlv(&tlv))
		display_tcg_tlv(&tlv);
		#ifdef DEBUG_MSG
			printf("I am in display tcg_tlv: \n");
       	        #endif	

	printf("Final pcr-10 should be ");
